...
let selectedAP = null;
let selectedClients = {};
let lightSettings = { r: 255, g: 255, b: 255, brightness: 100 };
...
function updateLightPreview() {
    ...
    const glow = $('#light-orb-glow');
    const status = $('#light-status');
    const controls = $('#light-controls');
    const hexDisplay = $('#color-hex');
    const toggle = $('#light-toggle');
    
    if (!preview) return;
    
    const hue = parseInt($('#light-hue')?.value || 0);
    const brightness = lightIsOn ? parseInt($('#light-brightness')?.value || 100) : 0;
    ...
    if (currentSaturation === 0) {
        ...
        rgb = hslToRgb(hue, currentSaturation, 60);
        hex = rgbToHex(rgb.r, rgb.g, rgb.b);
    }
    
    if (hexDisplay) hexDisplay.textContent = hex.toUpperCase();
    if (toggle) toggle.checked = lightIsOn;
    if (controls) controls.classList.toggle('active', lightIsOn);
    
    if (!lightIsOn || brightness === 0) {
        preview.style.background = '#27272a';
        if (glow) glow.classList.remove('active');
        if (status) status.textContent = 'Off';
    } else {
        preview.style.background = hex;
        if (glow) {
            glow.style.background = `radial-gradient(circle, rgba(${rgb.r},${rgb.g},${rgb.b},0.5) 0%, transparent 70%)`;
            glow.classList.add('active');
        }
        if (status) status.textContent = `${brightness}%`;
    }
}
...
function setColorTemp(type) {
    const temp = colorTemps[type];
    if (!temp) return;
    ...
    $('#light-hue').value = temp.hue;
    currentSaturation = temp.sat;
    updateLightPreview();
    applyLightSettings();
}

async function applyLightSettings() {
    if (!lightIsOn) {
        const res = await fetchJSON('/light', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ brightness: 0, rgb: { r: 0, g: 0, b: 0 } })
        });
        return;
    }
    
    const hue = parseInt($('#light-hue').value);
    const brightness = parseInt($('#light-brightness').value);
    let rgb;
    if (currentSaturation === 0) {
        rgb = { r: 255, g: 255, b: 255 };
    } else {
    ...
    lightSettings = { r: rgb.r, g: rgb.g, b: rgb.b, brightness };
}

async function loadLightStatus() {
    const res = await fetchJSON('/light/status');
    if (res) {
        lightIsOn = res.brightness > 0;
        const brightness = res.brightness || 100;
        currentSaturation = 0;
        
        $('#light-brightness').value = brightness;
        $('#brightness-value').textContent = brightness + '%';
        $('#light-hue').value = 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const debouncedApply = () => {
        ...
        applyTimeout = setTimeout(applyLightSettings, 150);
    };
    ...
    $('#light-hue')?.addEventListener('input', () => {
