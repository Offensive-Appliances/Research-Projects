...
static void peer_event_handler(peer_event_type_t event, const peer_info_t *peer) {
    switch (event) {
        ...
        case PEER_EVENT_DISCOVERED:
            if (peer) {
                ESP_LOGI(TAG, "New Legacy-HNL peer discovered: %s (%02x:%02x:%02x:%02x:%02x:%02x)",
        ...
        case PEER_EVENT_LOST:
            if (peer) {
                ESP_LOGI(TAG, "Legacy-HNL peer lost: %02x:%02x:%02x:%02x:%02x:%02x",
...
void wifi_init_softap() {
    ...
    esp_netif_set_hostname(sta_netif, "legacyhnl");
    ...
    esp_netif_set_hostname(ap_netif, "legacyhnl");
    mdns_service_init("legacyhnl");
