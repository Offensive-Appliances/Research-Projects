...
typedef enum {
    PEER_ROLE_UNKNOWN = 0,
    PEER_ROLE_LEADER,    // Owns legacyhnl.local
    PEER_ROLE_FOLLOWER   // Uses legacyhnl-XXXX.local
} peer_role_t;
...
/**
 * @brief Get this device's unique hostname (legacyhnl or legacyhnl-XXXX)
 * @return Pointer to hostname string
 */
const char* peer_discovery_get_hostname(void);

/**
 * @brief Get this device's AP SSID (Legacy-HNL or Legacy-HNL-XXXX)
 * @return Pointer to AP SSID string
 */
const char* peer_discovery_get_ap_ssid(void);

/**
 * @brief Scan for other Legacy-HNL APs and update role if needed
 ...
 * Used to detect other Legacy-HNL APs without initiating a dedicated scan
...
/**
 * @brief Check if we detected other Legacy-HNL APs via WiFi scanning
 * @return true if other Legacy-HNL APs were found
 */
